...
class ReportGenerator:
        ...
        try:
            ...
            if output_format.lower() == 'pdf':
                ...
                if wkhtmltopdf_path and os.path.exists(wkhtmltopdf_path):
                    ...
                    # Try without explicit path (system PATH)
    ...
    def _generate_timeline(self, violations, student_id):
        ...
        try:
            ...
            for violation in violations:
                ts_str = violation['timestamp']
                # Handle both timestamp formats used across the codebase
                for fmt in ("%Y%m%d_%H%M%S_%f", "%Y-%m-%dT%H:%M:%S.%f", "%Y-%m-%dT%H:%M:%S", "%Y-%m-%d %H:%M:%S"):
                    try:
